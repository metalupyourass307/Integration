...
df = pd.read_csv('/Users/laiyuxuan/Desktop/Interpolation/H/Single_Atom.txt', delim_whitespace=True, skiprows=2, header=None)
...
for i, u in enumerate(u_vals):
    ...
    if np.isnan(x_sol):
        x_sol = np.random.uniform(-1e-8, 1e-8)
    for j, v in enumerate(v_vals):
        y_sol = newton_raphson_y(v, x_sol)
        if np.isnan(y_sol):
            y_sol = np.random.uniform(-1e-8, 1e-8)
        for k, w in enumerate(w_vals):
            z_sol = newton_raphson_z(w, x_sol, y_sol)
            if np.isnan(z_sol):
                z_sol = np.random.uniform(-1e-8, 1e-8)
